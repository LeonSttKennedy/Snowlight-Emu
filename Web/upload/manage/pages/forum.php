...
if (isset($_POST['msg']))
{
	...
	mysql_query("INSERT INTO moderation_forum_threads (subject,message,poster,date,timestamp) VALUES ('" . $subject . "','" . $body . "','" . USER_NAME . "','" . date('j F Y h:i A') . "','" . time() . "')");
...
$getTopics = mysql_query("SELECT * FROM moderation_forum_threads ORDER BY timestamp DESC");
...
if (mysql_num_rows($getTopics) >= 1)
{
	while ($topic = mysql_fetch_assoc($getTopics))
	{
		...
		echo $topic['subject'] . '</b>&nbsp;';
		
		$rCount = mysql_result(mysql_query("SELECT COUNT(*) FROM moderation_forum_replies WHERE thread_id = '" . $topic['id'] . "'"), 0);
