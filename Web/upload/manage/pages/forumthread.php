...
{
	$i = intval($_GET['i']);
	$s = mysql_query("SELECT * FROM moderation_forum_threads WHERE id = '" . $i . "' LIMIT 1");
...
{	
	...
	mysql_query("INSERT INTO moderation_forum_replies (thread_id,poster,date,message) VALUES ('" . $t['id'] . "','" . USER_NAME . "','" . date('j F Y h:i A') . "','" . $msg . "')");
	
	if ($t['timestamp'] < 99999999999)
	{
		mysql_query("UPDATE moderation_forum_threads SET timestamp = '" . time() . "' WHERE id = '" . $t['id'] . "' LIMIT 1");
	}
...
if (isset($_POST['opt']))
{
	...
	switch ($opt)
	{
		case 'lock':
			...
			mysql_query("UPDATE moderation_forum_threads SET locked = '" . $newState . "' WHERE id = '" . $t['id'] . "' LIMIT 1");
			break;
		...
		case 'stick':
			...
			mysql_query("UPDATE moderation_forum_threads SET timestamp = '99999999999' WHERE id = '" . $t['id'] . "' LIMIT 1");
			break;
		...
		case 'del';
			...
			mysql_query("DELETE FROM moderation_forum_threads WHERE id = '" . $t['id'] . "' LIMIT 1");
			mysql_query("DELETE FROM moderation_forum_replies WHERE thread_id = '" . $t['id'] . "'");
			break;
	}
...
<h1>
	<?php if ($t['locked'] >= 1) { echo '<img src="images/locked.gif" alt="Locked" title="Thread locked" style="vertical-align: middle;">'; } ?>
	<?php if ($t['timestamp'] >= 99999999999) { echo '<img src="images/sticky.gif" alt="Sticky" title="Sticky topic" style="vertical-align: middle;">'; } ?>
	Discussion Board - "<?php echo $t['subject']; ?>"<br />
	<small style="font-weight: normal; font-size: 80%;"><?php echo $t['poster']; ?> on <?php echo $t['date']; ?> (<a href="index.php?_cmd=forum">Return to discussion board</a>)</small>
</h1>
...
$getReplies = mysql_query("SELECT * FROM moderation_forum_replies WHERE thread_id = '" . $t['id'] . "'");
...
if (mysql_num_rows($getReplies) >= 1)
{
	...
	while ($r = mysql_fetch_assoc($getReplies))
	{
		...
		echo '<p style="padding: 5px;">' . nl2br($r['message']) . '</p><br />';
