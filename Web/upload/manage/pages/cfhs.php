...
$get = mysql_query("SELECT * FROM moderation_tickets ORDER BY id DESC");

while ($user = mysql_fetch_assoc($get))
{
	echo '<tr>';
	echo '<td>' . $user['id'] . '</td>';
	echo '<td>' . formatType($user['category']) . '</td>';
	echo '<td>' . $user['status'] . '</td>';
	echo '<td>' . $user['reportee_user_id'] . '</td>';
	echo '<td>';
	
	if ($user['reported_user_id'] >= 1)
	{
		echo $user['reported_user_id'];
	}
	else
	{
		echo '-';
	...
	if ($user['moderator_user_id'] >= 1)
	{
		echo $user['moderator_user_id'];
	}
	else
	{
		echo '-';
